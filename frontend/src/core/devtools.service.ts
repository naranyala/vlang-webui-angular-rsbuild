...
export class DevToolsService {
   ...
   */
  async getStats(): Promise<DevToolsStats | null> {
    this.setState({ loading: true, error: null });

    try {
      const response = await this.api.call<DevToolsStats>('devtools.getStats');
  ...
  async getLogs(): Promise<LogEntry[]> {
    try {
      const response = await this.api.call<LogEntry[]>('devtools.getLogs');
      if (response.success && response.data) {
  ...
  async getErrors(): Promise<ErrorReport[]> {
    try {
      const response = await this.api.call<ErrorReport[]>('devtools.getErrors');
      if (response.success && response.data) {
  ...
  async getMetrics(): Promise<PerformanceMetric[]> {
    try {
      const response = await this.api.call<PerformanceMetric[]>('devtools.getMetrics');
      if (response.success && response.data) {
