...
import type { ErrorValue } from '../../types/error.types';
        ...
        <section class="error-modal" role="dialog" aria-modal="true" aria-label="Application error" (click)="$event.stopPropagation()">
          <header class="error-header">
            <div class="error-title-wrapper">
              <span class="error-icon">{{ getErrorIcon(error.code) }}</span>
          ...
          <div class="error-body">
            ...
            @if (getContextEntries(error.context || {}).length > 0) {
              <div class="error-context">
                <h4>Context</h4>
                <div class="context-grid">
                  @for (entry of getContextEntries(error.context || {}); track entry.key) {
          ...
          <footer class="error-footer">
            <div class="error-meta">
              <span class="error-code" [title]="error.code">
              ...
              <span class="error-timestamp" [title]="'Occurred at ' + (error.context?.timestamp || '')">
                {{ formatTimestamp(this.toString(error.context?.timestamp || this.now())) }}
              </span>
            </div>
            ...
            @if (error.details) {
              <details class="error-details-block">
                ...
                <div class="error-details-wrapper">
                  <pre class="error-details">{{ formatDetails(error.details) }}</pre>
                  <button class="copy-details-btn" (click)="copyDetails(error.details)" title="Copy to clipboard">
                    Copy
                  </button>
                </div>
              </details>
...
export class ErrorModalComponent {
  ...
  toString(value: number | string | undefined | null): string {
    return String(value || '');
  }
