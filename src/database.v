...
fn (mut db_service DatabaseService) load_users() {
	...
	db_service.users = json.decode([]User, data) or {
...
pub fn (mut db_service DatabaseService) get_all_users() []User {
	mut result := []User{}
...
pub fn (mut db_service DatabaseService) get_user_by_id(id int) ?User {
...
pub fn (mut db_service DatabaseService) get_user_by_email(email string) ?User {
	for user in db_service.users {
		if user.email == email {
			return user
		}
...
pub fn (mut db_service DatabaseService) create_user(user User) !User {
...
pub fn (mut db_service DatabaseService) update_user(id int, user User) !User {
	mut found_index := -1
...
pub fn (mut db_service DatabaseService) search_users(query string) []User {
	mut results := []User{}
...
pub fn (mut db_service DatabaseService) get_users_by_status(status string) []User {
	mut results := []User{}
	for user in db_service.users {
